...
- Функция принимает запросы только с адресов из `ALLOWED_ORIGINS`, обрезает длину полей и отсекает простых ботов скрытым полем. Это базовая защита: от целенаправленного спама она не спасёт. Дальше можно добавить Cloudflare Turnstile (капча) и правило ограничения частоты.
